package com.xwl.prisonbreak.util.poiExcel;

import com.alibaba.fastjson.JSONObject;
import com.xwl.prisonbreak.util.poiExcel.callBack.ExportSheetCallback;
import com.xwl.prisonbreak.util.poiExcel.callBack.ParseSheetCallback;
import com.xwl.prisonbreak.util.poiExcel.verify.AbstractCellVerify;
import com.xwl.prisonbreak.util.poiExcel.verify.AbstractVerifyBuidler;
import com.xwl.prisonbreak.util.poiExcel.verify.ImgVerify;
...
public class ExcelUtils {
    ...
    public static class ExportRules {
         ...
         */
        public ExportRules(String titile, String[] header, Column[] column, Boolean autoNum, HashMap<String, String> footerRules) {
            super();
         ...
         */
        public ExportRules(Map<String, String> headerRules, Column[] column, Boolean autoNum, Map<String, String> footerRules) {
            super();
            this.autoNum = autoNum;
            this.column = column;
    ...
    /**
     * 此方式只能用于遍历表头只有一行的表，且表头的行为第一行
     * 并且表头应该与实体属性同名，即实体类中有一个字段为name,表头也必须为name而不能是“姓名”！！
     *
     * 从excel中获取json字符串
     * 将json字符串解析成List<Object>: JSONObject.parseArray(json, XXX.class);XXX为你要解析成的对象
     * 只支持xsl
     * @param excelfile
     * @return 返回JSON字符串，可以通过JSON解析工具（如：阿里的fastjson）解析成对应的实体类
     */
    public static String getJsonStrFromExcel(MultipartFile excelfile) {

        POIFSFileSystem fs = null;
        FileOutputStream fileOut = null;
        // 表字段集合
        List<String> tablefielnames = new ArrayList<>();
        // result json
        ...
        try {
            ...
            // 列数
            int cellmaxnum = firstrow.getLastCellNum();
            // 行数
            long rowmaxnum = sheet.getLastRowNum();

            // 遍历列
            for (int i = 0; i < cellmaxnum; i++) {
                // 装载表字段
                tablefielnames.add(firstrow.getCell(i).getStringCellValue());
            }

            // 遍历行，下表为0的哪一行为表头
            for (int i = 1; i < rowmaxnum; i++) {
                Map<String, Object> temp = new HashMap<>();
                HSSFRow row = sheet.getRow(i);
                for (int j = 0; j < cellmaxnum; j++) {
                    String key = tablefielnames.get(j);
                    String val = "";
